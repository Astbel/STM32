...
/* USER CODE END PFP */
void Flash_Data(uint32_t Start_Addr,uint32_t End_Addr,uint32_t write_data);
/* Private user code ---------------------------------------------------------*/
/**
...
void Flash_Data(uint32_t Start_Addr,uint32_t End_Addr,uint32_t write_data)
{
    Uart_sendstring("Start write data to flash memory\r\n");
  ...
  /* Program the user Flash area word by word
    ...
    while (Address < End_Addr)
    {
        ...
        else
        {
            ...
            while (1)
            {
                ...
                Uart_sendstring("fail writing data\r\n");
            }
    ...
    HAL_FLASH_Lock();
    Flash_done_Flag =1;
    Uart_sendstring("End with Write memory !!!\r\n");
  /* Check if the programmed data is OK
    ...
    if (MemoryProgramStatus == 0)
    {
        ...
        Uart_sendstring("Done with Flash memory !!!\r\n");
    }
//   else
//   {
...
//Erase Data
void Erase_Data(uint32_t Start_Addr,uint32_t End_Addr,uint32_t write_data)
{
    Uart_sendstring("Erase memory\r\n");
    /* Unlock the Flash to enable the flash control register access *************/
    HAL_FLASH_Unlock();
...
//UART Send Data
void UART_Send_Data(uint32_t Start_Addr,uint32_t End_Addr,uint32_t write_data)
{
    Uart_sendstring("Writting memory\r\n");
    /* Unlock the Flash to enable the flash control register access *************/
    HAL_FLASH_Unlock();
